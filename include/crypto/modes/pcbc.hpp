...
class PCBCMode : public IBlockCipherMode {
private:
...
public:
    PCBCMode(std::shared_ptr<IBlockCipher> cipher, 
             std::unique_ptr<IPadding> padding = nullptr);
    
    CipherMode mode() const override { return CipherMode::PCBC; }
    std::string name() const override { return "PCBC"; }
